			...
			if (ctrlText ((findDisplay 63) displayCtrl 101) in [(localize "str_channel_global"),(localize "str_channel_side")]) then 
			{
				...
				if (disconnect_me == 0) then {
					if (isNil "warn_one") then {
						warn_one = true;
						//[format ["Please do not use voice on globalchat!"], "titleTextMessage", _target, false] spawn BIS_fnc_MP;
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["NO VOICE ON Global/Side Chat"] spawn DS_double_cut;
					};
				};
				...
				if (disconnect_me == 2) then {
					if (isNil "warn_three") then {
						warn_three = true;
						//[format ["LAST warning! NO GLOBAL VOICE CHAT\nYou will be kicked!"], "titleTextMessage", _target, false] spawn BIS_fnc_MP;
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["NO VOICE ON Global/Side Chat! LAST warning!"] spawn DS_double_cut;
						player setDamage 1;
					};
				};
