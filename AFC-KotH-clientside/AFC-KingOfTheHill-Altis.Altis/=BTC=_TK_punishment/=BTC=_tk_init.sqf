	...
	if (_name != name _body && BTC_vip find (name _killer) == -1) then
	{
		if (side _killer == BTC_side && _veh_body != _veh_killer) then
		{
			...
			_killer spawn 
			{
				...
				_action = player addAction [("<t color=""#ED2744"">") + ("Punish " + name _this) + "</t>","=BTC=_TK_punishment\=BTC=_punish_action.sqf",[name _this], -20, true, true, "", "true"];
				_timeout = time + 30;
				WaitUntil {uisleep 1; (_timeout < time)};
